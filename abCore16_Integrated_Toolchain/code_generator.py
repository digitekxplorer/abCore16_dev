# code_generator.py (with all bitwise operators)
...
class SALCodeGenerator:
    ...
    # --- (All helper methods from _initialize_temp_regs to visit_WhileNode are unchanged) ---
    ...
    def new_label(self, prefix="LBL"):
        self.label_counter += 1; return f"_{prefix.upper()}_{self.label_counter}"
    ...
    def _get_expression_type(self, node):
        ...
        if isinstance(node, UnaryOpNode) and node.op == '&': return 'pointer'
        if isinstance(node, UnaryOpNode) and node.op == '*': return 'word'
        if isinstance(node, BinaryOpNode) and node.op in ['+', '-']:
            ...
            if left_type == 'pointer' or right_type == 'pointer': return 'pointer'
        return 'word'
    ...
    def generate(self, ast_root_node):
        ...
        if hasattr(ast_root_node, 'statements'):
            for stmt in ast_root_node.statements:
                if isinstance(stmt, FunctionDefinitionNode):
                    ...
                    if func_name_upper in self.function_meta_map: raise Exception(
                        f"CodeGen Error: Function '{func_name_upper}' redefined.")
        ...
        if "MAIN" in self.function_meta_map: self.emit(
            f"CALL {self.function_meta_map['MAIN']['sal_label']}"); main_called_or_global_code_ran = True
        if main_called_or_global_code_ran:
            if not self.sal_code or self.sal_code[-1].strip().upper() != "HALT": self.emit(
                "HALT // Auto-HALT after main logic/global code")
        ...
        if self._currently_used_temps and CG_DEBUG_VERBOSE: print(
    ...
    def _calculate_local_storage_size_recursive(self, ast_node):
        ...
        if isinstance(ast_node, VarDeclNode):
            ...
            if is_in_function: size += ast_node.size * 2
    ...
    def visit_VarDeclNode(self, node):
        ...
        var_data_type = 'word'
        if node.init_expr_node: var_data_type = self._get_expression_type(node.init_expr_node)
    ...
    def visit_IdentifierNode(self, node):
        ...
        if name_upper in ["R0", "R1", "R2", "R3", "R4", "R5"]: return name_upper
        ...
        if symbol_info['type'] == 'global':
            ...
            self._free_temp(temp_reg); raise Exception(
                f"CodeGen Error: Unknown symbol type '{symbol_info['type']}' for '{node.name}' during lookup")
        if temp_reg is None or not isinstance(temp_reg, str): raise Exception(
    ...
    def visit_AssignmentNode(self, node):
        ...
        if isinstance(target_node, IdentifierNode):
            ...
            if value_type == 'pointer': self._update_symbol_type(target_node.name, 'pointer')
    ...
    def visit_FunctionDefinitionNode(self, node):
        ...
        if not func_meta: raise Exception(f"CodeGen Error: Meta info for func '{func_name_upper}' not found.")
        original_outer_context = self.current_function_context
        self.current_function_context = {'name': func_name_upper, 'sal_label': func_meta['sal_label'],
                                         'epilogue_label': func_meta['epilogue_label'], 'param_map': {},
        ...
        if self.sal_code:
            ...
            if last_sal_instr == "RET" or (
                    last_sal_instr.startswith("JMP") and self.current_function_context and last_sal_instr.endswith(
                self.current_function_context['epilogue_label'])): needs_implicit_jmp_to_epilogue = False
        if needs_implicit_jmp_to_epilogue and self.current_function_context: self.emit(
            f"JMP {self.current_function_context['epilogue_label']}")
        self.emit(f"{self.current_function_context['epilogue_label']}:")
        if local_storage_bytes_to_alloc > 0: self.emit(f"MOVTOSP {self.frame_pointer_reg}")
        ...
        if isinstance(condition_node, BinaryOpNode):
            ...
            if op in ['==', '!=', '<', '>', '<=', '>=']:
                ...
                if op == '==':
                    ...
                    lbl = self.new_label("SLT"); self.emit(f"JS {lbl}"); self.emit(
                        f"JO {true_branch_label}"); self.emit(f"JMP {false_branch_label}"); self.emit(
                        f"{lbl}:"); self.emit(f"JNO {true_branch_label}")
                elif op == '>=':
                    lbl = self.new_label("SGE"); self.emit(f"JS {lbl}"); self.emit(
                        f"JNO {true_branch_label}"); self.emit(f"JMP {false_branch_label}"); self.emit(
                        f"{lbl}:"); self.emit(f"JO {true_branch_label}")
                elif op == '>':
                    self.emit(f"JE {false_branch_label}"); lbl = self.new_label("SGT"); self.emit(
                        f"JS {lbl}"); self.emit(f"JNO {true_branch_label}"); self.emit(
                        f"JMP {false_branch_label}"); self.emit(f"{lbl}:"); self.emit(f"JO {true_branch_label}")
                elif op == '<=':
                    self.emit(f"JE {true_branch_label}"); lbl = self.new_label("SLE"); self.emit(
                        f"JS {lbl}"); self.emit(f"JO {true_branch_label}"); self.emit(
                        f"JMP {false_branch_label}"); self.emit(f"{lbl}:"); self.emit(f"JNO {true_branch_label}")
                self.emit(f"JMP {false_branch_label}");
                return
        elif isinstance(condition_node, UnaryOpNode) and condition_node.op == '!':
            self._generate_conditional_branch(condition_node.operand, false_branch_label, true_branch_label); return
    ...
    def visit_BinaryOpNode(self, node):
        ...
        if op in ['+', '-']:
            left_type = self._get_expression_type(node.left);
            right_type = self._get_expression_type(node.right)
            if (left_type == 'pointer' and right_type == 'word') or (left_type == 'word' and right_type == 'pointer'):
                self.emit(f"// --- Pointer Arithmetic ({op}) ---");
                reg_ptr = None;
                reg_int = None
                if left_type == 'pointer':
                    reg_ptr = node.left.accept(self); reg_int = node.right.accept(self)
                else:
                    reg_int = node.left.accept(self); reg_ptr = node.right.accept(self)
                self.emit(f"SHL {reg_int}, #1 // Scale integer for pointer arithmetic")
                if op == '+':
                    self.emit(f"ADD {reg_ptr}, {reg_int}")
                else:
                    if left_type != 'pointer': raise Exception(
        ...
        # -----------------
        # --- CHANGE #1: Add cases for OR and XOR ---
        # -----------------
        if op == '+':
    ...
    def visit_UnaryOpNode(self, node):
        ...
        if op == '&':
            ...
            if isinstance(operand, IdentifierNode):
                ...
                if symbol['data_type'] == 'pointer' and symbol['type'] in ['array', 'local_array']:
                    ...
                    self._free_temp(addr_reg); raise Exception(
        ...
        # -----------------
        # --- CHANGE #2: Add case for bitwise NOT (~) ---
        # -----------------
        if op == '-':
            temp_zero_reg = self._new_temp();
            self.emit(f"LOAD {temp_zero_reg}, #0");
    ...
    def visit_FunctionCallNode(self, node):
        ...
        for arg_expr_node in reversed(node.args_nodes):
            ...
            if arg_val_reg in self._compiler_temp_pool_master: temp_arg_regs_used.append(arg_val_reg)
        for temp_reg in temp_arg_regs_used: self._free_temp(temp_reg)
